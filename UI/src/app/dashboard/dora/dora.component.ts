...
export class DoraComponent implements OnInit {
  ...
  processKpiConfigData() {
    ...
    this.updatedConfigGlobalData?.forEach(
      (item) => (this.updatedConfigDataObj[item.kpiId] = { ...item }),
    );
  }
  ...
  receiveSharedData($event) {
    this.isTooltip = '';
    if (
      this.service.getSelectedLevel() &&
      this.service.getSelectedLevel()['hierarchyLevelName']
    ) {
      this.sprintsOverlayVisible =
        this.service.getSelectedLevel()['hierarchyLevelName'].toLowerCase() ===
        'project'
          ? true
          : false;
    }
    ...
    if (
      this.hierarchyLevel &&
      this.hierarchyLevel[+this.filterApplyData.level - 1]?.hierarchyLevelId ===
        'project'
    ) {
  ...
  groupJenkinsKpi(kpiIdsForCurrentBoard) {
    ...
    this.updatedConfigGlobalData?.forEach((obj) => {
      if (
        ...
        obj['kpiDetail'].kpiSource === 'Jenkins' &&
        obj['kpiDetail'].kpiCategory?.toLowerCase() == 'dora'
      ) {
      ...
      if (this.kpiSelectedFilterObj[kpiId]?.length > 1) {
        ...
        if (Array.isArray(this.kpiSelectedFilterObj[kpiId])) {
          for (let i = 0; i < this.kpiSelectedFilterObj[kpiId]?.length; i++) {
            ...
            tempArr[this.kpiSelectedFilterObj[kpiId][i]][0][
              'aggregationValue'
            ] = trendValueList?.filter(
              (x) => x['filter'] == this.kpiSelectedFilterObj[kpiId][i],
            )[0]?.value[0]?.aggregationValue;
          }
        } else {
        ...
        if (this.kpiSelectedFilterObj[kpiId]?.length > 0) {
          ...
          this.kpiChartData[kpiId][0]['aggregationValue'] =
            trendValueList?.filter(
              (x) => x['filter'] == this.kpiSelectedFilterObj[kpiId][0],
            )[0]?.value[0]?.aggregationValue;
        } else {
  ...
  setMaturityColor(kpiId, selectedFilter = null) {
    ...
    if (
      ...
      let maturityColor = selectedKPI.maturityLevel.filter(
        (level) => level.level === maturity,
      )[0]?.bgColor;
  ...
  calculateMaturity(kpiId, maturityRange) {
    if (maturityRange && maturityRange.length) {
      ...
      let findIncrementalOrDecrementalRange =
        this.findIncrementalOrDecrementalRange(maturityRange);
        ...
        case 'incremental':
          for (let i = 0; i < maturityRange.length; i++) {
            ...
            if (upperBound === Infinity) {
              ...
              if (
                aggregatedValue >= lowerBound &&
                aggregatedValue < upperBound
              ) {
                maturity = `M${i + 1}`;
                break;
              }
  ...
  findIncrementalOrDecrementalRange(range) {
    if (
      parseInt(range[1].split('-')[1] + 1) >
      parseInt(range[2].split('-')[1] + 1)
    ) {
      return 'decremental';
    } else {
      return 'incremental';
