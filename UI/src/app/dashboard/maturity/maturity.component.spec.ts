...
describe('MaturityComponent', () => {
  ...
  xit('should call kpi grouping methods on receiveing data for Scrum', () => {
    ...
    const spy = spyOn(service, 'getSelectedTab').and.returnValue(
      'kpi-maturity',
    );
    const spyongetMasterData = spyOn(service, 'getMasterData').and.returnValue(
      masterData,
    );
  ...
  xit('should call kpi grouping methods on receiveing data for Kanban', () => {
    ...
    // let spyDrawAreaChart =spyOn(component,'drawAreaChart');
    const groupingMethods = [
      'groupJenkinsKanbanKpi',
      'groupZypherKanbanKpi',
      'groupBitBucketKanbanKpi',
      'groupSonarKanbanKpi',
      'groupJiraKanbanKpi',
    ];
    const spyGroupingMthods = [];
  ...
  it('should make post call when kpi available for Sonar for Scrum', () => {
    ...
    const spyMasterData = spyOn(service, 'getMasterData').and.returnValue([
      'kpi17',
    ]);
  ...
  it('should make post call for each Kpi categories', fakeAsync(() => {
    const sonarKpiData = [
      {
        ...
        thresholdValue: 55,
        trendValueList: [],
        maturityRange: ['>=20', '20-40', '40-60', '60-80', '80-0'],
        groupId: 1,
      },
    ];
    ...
    const zypherKpiData = [
      {
        ...
        kpiSource: 'Zypher',
        trendValueList: [],
        maturityRange: ['0 - 20', '20-40', '40-60', '60-80', '80>='],
        groupId: 1,
      },
    ];
    const jiraKpiData = [
    ...
    const fakePayloads = [
      ...
      fakeZypherPayload,
      fakeJiraPayload,
      fakeBitbucketPayload,
    ];
    for (let i = 0; i < postMethods.length; i++) {
      httpService.postKpi = jasmine
        .createSpy()
        .and.returnValue(of(fakeResponses[i]));
      component[postMethods[i]](fakePayloads[i], sources[i]);
    }
    tick();
    expect(Object.keys(component.sonarKpiData).length).toEqual(0);
    expect(Object.keys(component.jenkinsKpiData).length).toEqual(
      jenkinsKpiData.length,
    );
    expect(Object.keys(component.zypherKpiData).length).toEqual(
      zypherKpiData.length,
    );
    expect(Object.keys(component.jiraKpiData).length).toEqual(
      jiraKpiData.length,
    );
    expect(Object.keys(component.bitBucketKpiData).length).toEqual(
      bitBucketKpiData.length,
    );
  ...
  it('should handle Tab Change', () => {
    ...
    const spyGetDashboardObject = spyOn(
      service,
      'getDashConfigData',
