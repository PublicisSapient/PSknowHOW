...
export class DeveloperComponent implements OnInit {
    ...
    this.subscriptions.push(
      this.service.passDataToDashboard.subscribe((sharedobject) => {
        if (
          ...
          sharedobject.selectedTab.toLowerCase() === 'developer'
        ) {
          this.allKpiArray = [];
          this.kpiChartData = {};
  ...
  receiveSharedData($event) {
    ...
    if (
      ...
      $event?.selectedTab?.toLowerCase() === 'developer'
    ) {
      ...
      if (
        ...
        if (this.filterData?.length && $event.makeAPICall) {
          ...
          if (this.configGlobalData?.length > 0) {
            this.configGlobalData = this.globalConfig[
              this.service.getSelectedType().toLowerCase() === 'kanban'
                ? 'kanban'
                : 'scrum'
            ].filter(
              (item) => item.boardName.toLowerCase() == 'developer',
            )[0]?.kpis;
  ...
  getChartData(kpiId, idx, aggregationType) {
    ...
    if (
      ...
      if (this.kpiSelectedFilterObj[kpiId]?.length > 1) {
        ...
        if (this.kpiSelectedFilterObj[kpiId]?.length > 0) {
          this.kpiChartData[kpiId] = trendValueList?.filter(
            (x) => x['filter'] == this.kpiSelectedFilterObj[kpiId][0].filter1,
          )[0]?.value;
          if (
            kpiId == 'kpi17' &&
      ...
      if (
        ...
        let initialKPIData = trendValueList?.filter(
          (x) =>
            x['filter1'] == this.kpiSelectedFilterObj[kpiId]['filter1'] &&
            x['filter2'] == this.kpiSelectedFilterObj[kpiId]['filter2'],
        )[0];
        ...
        if (trendValueList && trendValueList?.length > 0) {
          if (
            !Object.keys(this.kpiSelectedFilterObj[kpiId]).length ||
            this.kpiSelectedFilterObj[kpiId].hasOwnProperty('filter')
          ) {
            let initialKPIData = trendValueList?.filter(
              (x) => x['filter1'] == 'Overall' && x['filter2'] == 'Overall',
            )[0];
            this.kpiChartData[kpiId] = initialKPIData.value;
          }
        } else {
    ...
    if (kpiId === 'kpi3' || kpiId === 'kpi53') {
      ...
      if (
        ...
        this.kpiSelectedFilterObj[kpiId]?.length &&
        this.kpiSelectedFilterObj[kpiId][0]
      ) {
        columnHeaders.push({
          field: 'name',
        ...
        columnHeaders.push({
          field: 'value',
          header: this.kpiSelectedFilterObj[kpiId][0],
        });
