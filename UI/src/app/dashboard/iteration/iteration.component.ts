...
export class IterationComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscriptions.push(
      this.service.passDataToDashboard.subscribe((sharedobject) => {
    ...
    this.subscriptions.push(
      this.service.globalDashConfigData.subscribe((globalConfig) => {
        if (globalConfig && this.sharedObject) {
          this.configGlobalData = globalConfig['scrum'].filter(
            (item) => item.boardName.toLowerCase() == 'iteration',
          )[0]?.kpis;
  ...
  processKpiConfigData() {
    ...
    this.commitmentReliabilityKpi = this.updatedConfigGlobalData.filter(
      (kpi) => kpi.kpiId === 'kpi120',
    )[0];
    this.upDatedConfigData = this.updatedConfigGlobalData.filter(
      (kpi) => kpi.kpiId !== 'kpi121' && kpi.isEnabled,
    );
    ...
    if (
      this.upDatedConfigData?.length === 0 &&
      !this.commitmentReliabilityKpi?.isEnabled
    ) {
      this.noKpis = true;
    } else {
      this.noKpis = false;
    ...
    this.navigationTabs.forEach((tabDetails) => {
      if (tabDetails['width'] === 'half') {
        ...
        tabDetails['kpis'].forEach((kpiDetails) => {
          if (
            kpiDetails.kpiDetail.kpiWidth &&
            kpiDetails.kpiDetail.kpiWidth === 100
          ) {
        ...
        tabDetails['kpiPart1'] = halfWithKpis.slice(
          0,
          middleIndex + (dataLength % 2),
        );
        tabDetails['kpiPart2'] = halfWithKpis.slice(
          middleIndex + (dataLength % 2),
        );
  ...
  receiveSharedData($event) {
    if (this.service.getDashConfigData()) {
      this.activeIndex = 0;
      this.configGlobalData = this.service
        .getDashConfigData()
        ['scrum']?.filter(
          (item) => item.boardName.toLowerCase() == 'iteration',
        )[0]?.kpis;
              ...
              switch (this.activeIndex) {
                case 2:
                  kpiIdsForCurrentBoard = this.configGlobalData
                    ...
                    .filter((kpiId) => kpiId === 'kpi154');
                  break;
                case 1:
                  kpiIdsForCurrentBoard = this.configGlobalData
                    ?.map((kpiDetails) => kpiDetails.kpiId)
                    .filter((kpiId) => kpiId === 'kpi125');
                  break;
                default:
                  kpiIdsForCurrentBoard = this.configGlobalData
                    ?.map((kpiDetails) => kpiDetails.kpiId)
                    .filter(
                      (kpiId) => kpiId !== 'kpi125' && kpiId !== 'kpi154',
                    );
                  break;
              }
              const selectedSprint = this.filterData?.filter(
                (x) =>
                  x.nodeId == this.filterApplyData?.selectedMap['sprint'][0],
              )[0];
              if (selectedSprint) {
                this.selectedProjectId = selectedSprint.nodeId?.substring(
                  selectedSprint.nodeId.lastIndexOf('_') + 1,
                  selectedSprint.nodeId.length,
                );
                this.checkForAssigneeDataAndSetupTabs();
  ...
  checkForAssigneeDataAndSetupTabs() {
    this.httpService.getProjectListData().subscribe(async (responseList) => {
      let selectedProject = responseList[0].data.filter(
      ...
      let showDailyStandup = await this.featureFlagService.isFeatureEnabled(
        'DAILY_STANDUP',
      );
      if (
        this.service.currentSelectedSprint?.sprintState.toLowerCase() ===
          'active' &&
        showDSVorNot
      ) {
        // && showDailyStandup) {
      ...
      if (
        this.navigationTabs.filter((tab) => tab['label'] === 'Daily Standup')
          .length
      ) {
        this.dailyStandupData = this.navigationTabs.filter(
          (tab) => tab['label'] === 'Daily Standup',
        )[0]['kpis'];
      }
    });
  }
  ...
  groupJiraKpi(kpiIdsForCurrentBoard) {
    ...
    this.updatedConfigGlobalData.forEach((obj) => {
      ...
      if (
        ...
        obj['kpiDetail'].kpiCategory == 'Iteration' &&
        obj.kpiId !== 'kpi154' &&
        obj.kpiId !== 'kpi125'
      ) {
    ...
    this.jiraKpiRequest = this.httpService
      ...
      .subscribe((getData) => {
        if (getData !== null && getData[0] !== 'error' && !getData['error']) {
          ...
          postData.kpiList.forEach((element) => {
            this.loaderJiraArray.splice(
              this.loaderJiraArray.indexOf(element.kpiId),
              1,
            );
          });
        }
  ...
  applyAggregationForChart(arr) {
    ...
    for (let i = 1; i < arr.length; i++) {
      for (let j = 0; j < arr[i].value.length; j++) {
        ...
        aggregatedArr.value[j].hoverValue = {
          ...aggregatedArr.value[j].hoverValue,
          ...arr[i].value[j].hoverValue,
        };
      }
    }
  ...
  applyAggregationLogic(arr) {
    ...
    const evalvateExpression = aggregatedArr[0]['data'].filter((el) =>
      el.hasOwnProperty('expressions'),
    );
  ...
  getChartData(kpiId, idx, aggregationType?) {
        ...
        for (let i = 0; i < filters?.length; i++) {
          preAggregatedValues = [
            ...
            ...trendValueList?.filter(
              (x) => x['filter1'].toLowerCase() === filters[i].toLowerCase(),
            ),
          ];
        }
    ...
    if (kpiId === 'kpi121') {
      const iterationConfigData = {
        ...
        capacity: {
          kpiInfo: this.updatedConfigGlobalData.find(
            (kpi) => kpi.kpiId === kpiId,
          )?.kpiDetail?.kpiInfo,
  ...
  createAllKpiArray(data) {
    for (const key in data) {
      ...
      if (
        ...
        this.setFilterValueIfAlreadyHaveBackup(
          data[key]?.kpiId,
          {},
          ['Overall'],
          filters,
        ...
        const formType = this.updatedConfigGlobalData?.filter(
          (x) => x.kpiId == data[key]?.kpiId,
        )[0]?.kpiDetail?.kpiFilter;
        if (formType?.toLowerCase() == 'radiobutton') {
          this.setFilterValueIfAlreadyHaveBackup(data[key]?.kpiId, {}, [
            this.kpiDropdowns[data[key]?.kpiId][0]?.options[0],
          ]);
  ...
  setFilterValueIfAlreadyHaveBackup(
    kpiId,
    refreshValue,
    initialValue,
    filters?,
    ...
    this.kpiSelectedFilterObj =
      this.helperService.setFilterValueIfAlreadyHaveBackup(
        kpiId,
        this.kpiSelectedFilterObj,
        'iteration',
        refreshValue,
        initialValue,
        this.filterApplyData['ids'][0],
        filters,
      );
    ...
    if (idx != -1) {
      ...
      if (filters && Object.keys(filters).length !== 0) {
        ...
        for (let i = 0; i < trendValueList?.length; i++) {
          let ifExist = dropdownArr.findIndex(
        ...
        if (
          ...
          const index = dropdownArr?.findIndex(
  ...
  handleSelectedOption(event, kpi) {
    ...
    this.helperService.createBackupOfFiltersSelection(
      this.kpiSelectedFilterObj,
      'iteration',
      this.filterApplyData['ids'][0],
    );
  ...
  handleArrowClick(kpi, label, tableValues) {
    ...
    if (this.allKpiArray[idx]?.trendValueList?.value[0]?.markerInfo) {
      for (const key in this.allKpiArray[idx]?.trendValueList?.value[0]
        ?.markerInfo) {
        this.markerInfo.push({
          color: key,
          info: this.allKpiArray[idx]?.trendValueList?.value[0]?.markerInfo[
            key
          ],
        });
      }
    }
    this.excludeColumns = this.allKpiArray[idx]?.trendValueList?.value[0]
      ?.metaDataColumns
      ? this.allKpiArray[idx]?.trendValueList?.value[0]?.metaDataColumns
      : [];
    this.httpService
  ...
  generateTableColumnsFilterData(tableValue) {
    ...
    this.modalDetails['tableHeadings'] = this.modalDetails[
      'tableHeadings'
    ].filter((colName) => !this.excludeColumns.includes(colName));
  ...
  saveKpiColumnsConfig(selectedColumns: any[], action: string) {
    ...
    postData['basicProjectConfigId'] =
      this.service.selectedTrends[0].basicProjectConfigId;
    ...
    this.tableHeaders = postData['kpiColumnDetails'].map(
      (col) => col.columnName,
    );
    if (action === 'SAVE') {
  ...
  generateTableColumnData() {
    this.modalDetails['tableHeadings'].forEach((colName) => {
      ...
      ].map((colData) => ({ name: colData, value: colData }));
  ...
  generateExcel(exportMode) {
    ...
    if (exportMode === 'all') {
      ...
      excelData =
        this.tableComponent?.filteredValue || this.modalDetails['tableValues'];
  ...
  drop(event: CdkDragDrop<string[]>, updatedContainer) {
    const capacityKpi = this.updatedConfigGlobalData.find(
      (kpi) => kpi.kpiId === 'kpi121',
    );
    this.helperService.drop(
      event,
      updatedContainer,
      this.navigationTabs,
  ...
  async getKpiCommentsCount(kpiId?) {
    ...
    this.kpiCommentsCountObj = await this.helperService.getKpiCommentsCount(
      this.kpiCommentsCountObj,
      nodes,
      level,
      nodeChildId,
  ...
  handleTabChange(e) {
    let index = e.index;
    if (index === 1) {
      let kpi125Data = this.configGlobalData?.filter(
        (kpi) => kpi.kpiId === 'kpi125',
      )[0];
      this.kpiJira = this.helperService.groupKpiFromMaster(
        'Jira',
        false,
    ...
    if (index === 2) {
      let kpi154Data = this.configGlobalData?.filter(
