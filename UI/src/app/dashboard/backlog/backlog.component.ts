...
export class BacklogComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(
      this.service.passDataToDashboard
        ...
        .subscribe((sharedobject) => {
          if (
            sharedobject?.filterData?.length &&
            sharedobject.selectedTab.toLowerCase() === 'backlog'
          ) {
            this.allKpiArray = [];
            this.kpiChartData = {};
  ...
  processKpiConfigData() {
    ...
    for (let i = 0; i < this.configGlobalData?.length; i++) {
      if (
        this.configGlobalData[i]?.shown === false &&
        this.configGlobalData[i]?.isEnabled === true
      ) {
        this.kpiConfigData[this.configGlobalData[i]?.kpiId] =
          this.configGlobalData[i]?.shown;
      } else {
        this.kpiConfigData[this.configGlobalData[i]?.kpiId] =
          this.configGlobalData[i]?.isEnabled;
      }
    }
    ...
    const kpi3Index = this.updatedConfigGlobalData.findIndex(
      (kpi) => kpi.kpiId === 'kpi3',
    );
    ...
    if (kpi3Index >= 0) {
      this.leadTime = this.updatedConfigGlobalData.find(
        (kpi) => kpi.kpiId === 'kpi3',
      );
    } else {
    ...
    const showKpisCount = Object.values(this.kpiConfigData).filter(
      (item) => item === true,
    )?.length;
    if (showKpisCount === 0) {
      this.noKpis = true;
  ...
  groupJiraKpi(kpiIdsForCurrentBoard) {
    ...
    this.updatedConfigGlobalData?.forEach((obj) => {
      if (
        ...
        obj?.['kpiDetail'].kpiCategory == 'Backlog'
      ) {
    ...
    groupIdSet.forEach((groupId) => {
      if (groupId) {
        const currentPayload = this.helperService.groupKpiFromMaster(
          'Jira',
          false,
        ...
        if (Object.keys(this.kpiJira).length) {
          this.kpiJira = {
            ...currentPayload,
            kpiList: currentPayload.kpiList.concat(this.kpiJira.kpiList),
          };
        } else {
    ...
    this.jiraKpiRequest = this.httpService
      ...
      .subscribe((getData) => {
        if (getData !== null && getData[0] !== 'error' && !getData['error']) {
          ...
          const localVarKpi =
            localVariable['kpi127'] ||
            localVariable['kpi170'] ||
            localVariable['kpi3'];
          if (localVarKpi) {
              ...
              localVarKpi.trendValueList.forEach((trendElem) => {
                trendElem.value.forEach((valElem) => {
                  if (
                    ...
                    localVarKpi.xAxisValues.length === 5
                  ) {
  ...
  createAllKpiArray(data) {
    for (const key in data) {
      ...
      if (
        this.updatedConfigGlobalData.find((kpi) => kpi?.kpiId === key)
          ?.kpiDetail?.chartType
      ) {
        if (
          trendValueList?.length > 0 &&
  ...
  getkpi171Data(kpiId, trendValueList) {
    ...
    if (durationChanged) {
      ...
      const kpi171 = kpi171Payload.kpiList.filter(
      ...
      this.httpService
        .postKpiNonTrend(kpi171Payload, 'jira')
        .subscribe((data) => {
  ...
  kpi171Check(kpiId, preAggregatedValues) {
    if (kpiId === 'kpi171') {
      ...
      let kpi3preAggregatedValues = JSON.parse(
        JSON.stringify(preAggregatedValues),
      );
      kpi3preAggregatedValues = kpi3preAggregatedValues.map((filterData) => {
        return {
          ...filterData,
          data: filterData.data.map((labelData) => ({
            ...labelData,
            value: labelData.value * labelData.value1,
          })),
        };
      });
  ...
  createTrendData(kpiId) {
    const kpiDetail = this.configGlobalData.find(
      (details) => details.kpiId == kpiId,
    );
  ...
  checkLatestAndTrendValue(kpiData, item) {
    ...
    if (item?.value?.length > 0) {
      let tempVal = item?.value[item?.value?.length - 1]?.lineValue
