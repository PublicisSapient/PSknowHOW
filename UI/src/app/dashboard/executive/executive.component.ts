    ...
    checkMaturity(item) {
      ...
      checkLatestAndTrendValue(kpiData, item){
        ...
        if(item?.value?.length > 0){
            ...
            let unit = kpiData?.kpiDetail?.kpiUnit?.toLowerCase() != 'number' && kpiData?.kpiDetail?.kpiUnit?.toLowerCase() != 'stories' && kpiData?.kpiDetail?.kpiUnit?.toLowerCase() != 'tickets'? kpiData?.kpiDetail?.kpiUnit : '';
