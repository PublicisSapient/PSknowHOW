...
export class ExecutiveComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscriptions.push(
      this.service.onTypeOrTabRefresh.subscribe((data) => {
        ...
        this.kanbanActivated =
          this.selectedtype.toLowerCase() === 'kanban' ? true : false;
      }),
    );
    ...
    this.subscriptions.push(
      this.service.globalDashConfigData.subscribe((globalConfig) => {
        this.configGlobalData = globalConfig[
          this.kanbanActivated ? 'kanban' : 'scrum'
        ].filter(
          (item) =>
  ...
  processKpiConfigData() {
    ...
    const shownKpis = this.configGlobalData?.filter(
      (item) => item.shown && item.isEnabled,
    );
  ...
  ngOnInit() {
    if (this.service.getFilterObject()) {
      this.serviceObject = JSON.parse(
        JSON.stringify(this.service.getFilterObject()),
      );
    }
  ...
  receiveSharedData($event) {
    ...
    if (
      this.service.getDashConfigData() &&
      Object.keys(this.service.getDashConfigData()).length > 0 &&
      $event?.selectedTab?.toLowerCase() !== 'iteration'
    ) {
      this.configGlobalData = this.service
        .getDashConfigData()
        [this.kanbanActivated ? 'kanban' : 'scrum'].filter(
          (item) =>
            item.boardName.toLowerCase() ===
      ...
      if (
        JSON.stringify(this.filterApplyData) !==
          JSON.stringify($event.filterApplyData) ||
        ...
        const kpiIdsForCurrentBoard = this.updatedConfigGlobalData?.map(
        ...
        if (this.filterData?.length && $event.makeAPICall) {
          ...
          if (this.configGlobalData?.length > 0) {
            ...
            if (this.service.getSelectedType().toLowerCase() === 'kanban') {
              this.configGlobalData = this.service
                .getDashConfigData()
                [this.selectedtype.toLowerCase()].filter(
                  (item) =>
                    item.boardName.toLowerCase() ===
    ...
    this.jiraKpiRequest = this.httpService
      ...
      .subscribe((getData) => {
        if (getData !== null && getData[0] !== 'error' && !getData['error']) {
          ...
          const localVariable = this.helperService.createKpiWiseId(getData);
          for (const kpi in localVariable) {
            this.loaderJiraKanbanArray.splice(
              this.loaderJiraKanbanArray.indexOf(kpi),
              1,
            );
          }
          if (localVariable['kpi997']) {
            localVariable['kpi997'] = this.kpi997Check(localVariable);
          }
          this.jiraKpiData = Object.assign({}, this.jiraKpiData, localVariable);
          this.createAllKpiArray(localVariable);
        } else {
          ...
          postData.kpiList.forEach((element) => {
            this.loaderJiraKanbanArray.splice(
              this.loaderJiraKanbanArray.indexOf(element.kpiId),
              1,
            );
          });
        }
  ...
  kpi997Check(localVariable) {
    if (
      localVariable['kpi997'].trendValueList &&
      localVariable['kpi997'].xAxisValues
    ) {
      localVariable['kpi997'].trendValueList.forEach((trendElem) => {
        trendElem.value.forEach((valElem) => {
          if (
            valElem.value.length === 5 &&
            localVariable['kpi997'].xAxisValues.length === 5
          ) {
  ...
  getChartData(kpiId, idx, aggregationType) {
      ...
      if (this.kpiSelectedFilterObj[kpiId]?.length > 1) {
        if (kpiId === 'kpi17') {
          ...
          if (Array.isArray(this.kpiSelectedFilterObj[kpiId])) {
            ...
            tempArr[this.kpiSelectedFilterObj[kpiId]] = trendValueList?.filter(
        ...
        if (this.kpiSelectedFilterObj[kpiId]?.length > 0) {
          this.kpiChartData[kpiId] = trendValueList?.filter(
            (x) => x['filter'] == this.kpiSelectedFilterObj[kpiId][0],
          )[0]?.value;
          if (
            kpiId == 'kpi17' &&
    ...
    // For kpi3 and kpi53 generating table column headers and table data- no longer required
    ...
    if (trendValueList?.length > 0) {
      ...
      filtersApplied.forEach((hierarchyName) => {
        ...
        let chosenItem = iterativeEle?.filter(
          (item) => item['data'] == hierarchyName,
        )[0];

        let trendData = this.kpiTrendsObj[kpiId]?.filter(
          (x) =>
            x['hierarchyName']?.toLowerCase() == hierarchyName?.toLowerCase(),
        )[0];
            ...
            obj[i + 1] =
              val > 0
                ? Math.round(val * 10) / 10 +
                  (trendData?.kpiUnit ? ' ' + trendData?.kpiUnit : '')
        ...
        if (enabledKpi?.isEnabled && enabledKpi?.shown) {
          this.kpiTableDataObj[hierarchyName] = [
            ...this.kpiTableDataObj[hierarchyName],
            obj,
          ];
        }
    ...
    for (const key in data) {
      ...
      if (
        ...
        if (formType?.toLowerCase() == 'radiobutton') {
          this.kpiSelectedFilterObj[data[key]?.kpiId]?.push(
    ...
    if (arr?.length > 0) {
      ...
      for (let i = 0; i < arr?.length; i++) {
        for (const key in this.colorObj) {
          if (
            kpiId == 'kpi17' &&
            this.colorObj[key]?.nodeName == arr[i].value[0].sprojectName
          ) {
            ...
            this.chartColorList[kpiId].push(this.colorObj[key]?.color);
            finalArr.push(
              arr.filter((a) => a.data === this.colorObj[key].nodeName)[0],
            );
          }
        }
      }
      ...
      if (
        ...
        for (let i = 0; i < trendValueList?.length; i++) {
          for (let key in this.colorObj) {
            let kpiFilter = trendValueList[i]?.value?.findIndex(
              (x) => this.colorObj[key]?.nodeName == x.data,
            );
            if (kpiFilter != -1) {
              let ifExist = trendValueList[i]?.filter1
                ? optionsArr.findIndex((x) => x == trendValueList[i]?.filter1)
                : optionsArr.findIndex((x) => x == trendValueList[i]?.filter);
              if (ifExist == -1) {
                optionsArr?.push(
                  trendValueList[i]?.filter1
                    ? trendValueList[i]?.filter1
                    : trendValueList[i]?.filter,
                );
              }
              if (trendValueList[i]?.hasOwnProperty('filter2')) {
                let ifF1Exist = optionsArr2.findIndex(
                  (x) => x == trendValueList[i]?.filter2,
                );
  ...
  getLastConfigurableTrendingListData(KpiData) {
    if (
      this.tooltip &&
      this.tooltip.sprintCountForKpiCalculation !== undefined
    ) {
      if (
        !(
          this.filterApplyData['label'] === 'sqd' &&
        ...
        KpiData.map((kpiList) => {
          kpiList.trendValueList?.map((trendData) => {
            if (
              trendData.hasOwnProperty('filter') ||
              trendData.hasOwnProperty('filter1')
            ) {
              trendData?.value.map((projectWiseData) => {
                const valueLength = projectWiseData.value.length;
                if (valueLength > this.tooltip.sprintCountForKpiCalculation) {
                  projectWiseData.value = projectWiseData.value.splice(
                    -this.tooltip.sprintCountForKpiCalculation,
                  );
                }
              });
