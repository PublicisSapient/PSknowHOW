...
describe('KpiCardComponent', () => {
  ...
  it('should showFilterTooltip', () => {
    ...
    expect(component.filterMultiSelectOptionsData['details'][1].length).toEqual(
      component.filterOptions[1].length,
    );
    component.showFilterTooltip(false);
    expect(Object.keys(component.filterMultiSelectOptionsData).length).toEqual(
      0,
    );
  });
  ...
  it('should set loading to false and noData to true when fieldMappingConfig length is 0', () => {
    ...
    spyOn(httpService, 'getKPIFieldMappingConfig').and.returnValue(
      of({ success: true, data: { fieldConfiguration: fieldMappingConfig } }),
    );
  ...
  it('should handle no data for KPI field mapping config', fakeAsync(() => {
    ...
    // Assert
    expect(httpService.getKPIFieldMappingConfig).toHaveBeenCalledWith(
      `${selectedTrend[0]?.basicProjectConfigId}/${component.kpiData?.kpiId}`,
    );
    expect(component.loading).toBeFalse();
    expect(component.noData).toBeTrue();
    expect(component.displayConfigModel).toBeTrue();
    expect(component.fieldMappingConfig).toEqual([]);
    expect(component.selectedToolConfig).toEqual([
      { id: '456', toolName: 'jira' },
    ]);
  }));
