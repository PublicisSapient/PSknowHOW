...
describe('LoginComponent', () => {
  ...
  const fakeInvalidLogin = {
    timestamp: 1567511436517,
    status: 401,
    error: 'Unauthorized',
    message:
      'Authentication Failed: Login Failed: The username or password entered is incorrect',
    path: '/api/login',
  };
  ...
  it('should perform login successfully', () => {
    let data = {
      ...
      body: {
        ...
        'X-Authentication-Token':
          'eyJhbGciOiJIUzUxMiJ9.eyJzdWIiOiJTVVBFUkFETUlOIiwiZGV0YWlscyI6IlNUQU5EQVJEIiwicm9sZXMiOlsiUk9MRV9TVVBFUkFETUlOIl0sImV4cCI6MTY4OTE0Nzc1OX0.y9uAnMjyNfsqeCxHXQb2zX6akOD_kAeM2AmmbHEyrPcAi7eKtS6yyChwtLQ_BsNM3u56ChdovxBXjNA2LjdLyQ',
        authorities: ['ROLE_SUPERADMIN'],
      },
    };
    const loginType = 'AD';
    ...
    sharedService.setCurrentUserDetails({ projectsAccess: [] });
    sharedService.setCurrentUserDetails({ authorities: ['ROLE_SUPERADMIN'] });
    const isRedirect = spyOn(component, 'redirectToProfile').and.returnValue(
      false,
    );
