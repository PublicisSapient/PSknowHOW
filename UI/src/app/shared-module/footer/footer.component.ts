...
export class FooterComponent implements OnInit {
  ...
  styleObj(): object {
    ...
    if (
      urlArray.includes('Help') ||
      urlArray.includes('Config') ||
      urlArray.includes('Error') ||
      urlArray[urlArray.length - 1].includes('login') ||
      urlArray.includes('register') ||
      localStorage.getItem('newUI')
    ) {
      marginLeft = '0rem';
    }
