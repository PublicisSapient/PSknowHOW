...
export class ProjectFilterComponent implements OnInit {
  ...
  filterData(event, filterType, filterValue) {
    ...
    if (
      !this.selectedVal[filterType] ||
      !this.selectedVal[filterType].filter((f) => f.code === filterValue).length
    ) {
      const obj = {
        name: filterValue,
        code: filterValue,
      ...
      this.valueRemoved['val'] = this.selectedVal[filterType].splice(
        this.selectedVal[filterType].indexOf(
          this.selectedVal[filterType].filter((f) => f.code === filterValue)[0],
        ),
        1,
      );
    ...
    if (Object.keys(this.selectedVal).length) {
      Object.keys(this.selectedVal).forEach((filter) => {
        if (this.selectedVal[filter] && this.selectedVal[filter].length) {
          this.selectedValTemplateValue[filter] = this.selectedVal[filter]
            ?.map((s) => s.code)
            .join(', ');
