...
export class AdditionalFilterFieldComponent implements OnInit {
  ...
  generateAdditionalFilterMappings() {
    ...
    this.selectedFieldMapping = this.sharedService
      .getSelectedFieldMapping()
      ?.fieldMappingResponses?.find(
        (ele) => ele.fieldName === 'additionalFilterConfig',
      );
    if (
      ...
      const additionalFilters = this.filterHierarchy?.filter(
        (filter) =>
          filter.level >
          this.filterHierarchy?.filter(
            (f) => f.hierarchyLevelId === 'sprint',
          )[0].level,
      );

      additionalFilterMappings?.forEach((element) => {
        this.additionalFiltersArray.push({
          name: additionalFilters.filter(
            (f) => f.hierarchyLevelId === element.filterId,
        ...
        if (element['identifyFrom']?.length) {
          if (
            !this.fieldMappingForm.controls[element.filterId + 'Identifier']
          ) {
            this.fieldMappingForm.addControl(
              element.filterId + 'Identifier',
              this.formBuilder.control(''),
            );
            this.fieldMappingForm.controls[
              element.filterId + 'Identifier'
            ].setValue(element['identifyFrom']);
          }
        }
        if (element['identifyFrom'] === 'CustomField') {
          if (
            !this.fieldMappingForm.controls[
              element.filterId + 'IdentSingleValue'
            ]
          ) {
            this.fieldMappingForm.addControl(
              element.filterId + 'IdentSingleValue',
            ...
            this.fieldMappingForm.controls[
              element.filterId + 'IdentSingleValue'
            ].setValue(element['identificationField']);
          }
        } else {
          if (
            !this.fieldMappingForm.controls[
              element.filterId + 'IdentMultiValue'
            ]
          ) {
            this.fieldMappingForm.addControl(
              element.filterId + 'IdentMultiValue',
  ...
  addAdditionalFilterOptions() {
    ...
    const additionalFilters = this.filterHierarchy?.filter(
      (filter) =>
        filter.level >
        this.filterHierarchy?.filter((f) => f.hierarchyLevelId === 'sprint')[0]
          ?.level,
    );
  ...
  addAdditionalFilterMappings() {
    if (
      !this.additionalFiltersArray.filter(
        (filter) => filter.name === this.additionalFilterIdentifier.name,
      ).length
    ) {
  ...
  changeControl(event, additionalFilterIdentifier) {
    if (event.value === 'Component' || event.value === 'Labels') {
      if (
        !this.fieldMappingForm.controls[
          additionalFilterIdentifier.code + 'IdentMultiValue'
        ]
      ) {
        this.fieldMappingForm.addControl(
          additionalFilterIdentifier.code + 'IdentMultiValue',
      ...
      if (
        !this.fieldMappingForm.controls[
          additionalFilterIdentifier.code + 'IdentSingleValue'
        ]
      ) {
        this.fieldMappingForm.addControl(
          additionalFilterIdentifier.code + 'IdentSingleValue',
  ...
  removeAdditionFilterMapping(filter) {
    ...
    this.additionalFiltersArray = this.additionalFiltersArray.filter(
  ...
  handleAdditionalFilters(): any {
    ...
    additionalFilters?.forEach((element) => {
      if (
        submitData[element.hierarchyLevelId + 'Identifier'] &&
        submitData[element.hierarchyLevelId + 'Identifier'].length
      ) {
        const additionalFilterObj = {};
        additionalFilterObj['filterId'] = element.hierarchyLevelId;
        additionalFilterObj['identifyFrom'] =
          submitData[element.hierarchyLevelId + 'Identifier'];
        if (additionalFilterObj['identifyFrom'] === 'CustomField') {
          additionalFilterObj['identificationField'] =
            submitData[element.hierarchyLevelId + 'IdentSingleValue'];
          additionalFilterObj['values'] = [];
        } else {
          additionalFilterObj['identificationField'] = '';
          additionalFilterObj['values'] = submitData[
            element.hierarchyLevelId + 'IdentMultiValue'
          ]
            ? submitData[element.hierarchyLevelId + 'IdentMultiValue']
            : [];
        }
      ...
      if (this.fieldMappingForm.controls[this.selectedField].value) {
        this.selectedValue = this.fieldMappingMultiSelectValues.filter(
          (fieldMappingMultiSelectValue) =>
            ...
            this.fieldMappingForm.controls[this.selectedField].value,
        );
  ...
  saveDialog() {
    if (this.singleSelectionDropdown) {
      if (this.selectedValue.length) {
        this.fieldMappingForm.controls[this.selectedField].setValue(
          this.selectedValue,
        );
      }
    }
