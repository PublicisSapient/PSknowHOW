...
export class HelperService {
      ...
      if (condition) {
        ...
        if (obj.hasOwnProperty('isEnabled') && obj.hasOwnProperty('shown')) {
          if (obj.isEnabled && obj.shown) {
            if (
              !kpiRequestObject.kpiList.filter((kpi) => kpi.kpiId === obj.kpiId)
                ?.length
            ) {
  ...
  createBackupOfFiltersSelection(filterbackup, tab, subFilter) {
    ...
    if (tab === 'backlog') {
      let tabSpecfic =
        savedDetails['kpiFilters'] && savedDetails['kpiFilters'][tab]
          ? savedDetails['kpiFilters'][tab]
          : {};
      savedDetails = {
        ...savedDetails,
        kpiFilters: {
          ...savedDetails['kpiFilters'],
          ...{ [tab]: { ...tabSpecfic, ...filterbackup } },
        },
      };
    } else {
      const subFilterValues =
        savedDetails['kpiFilters'] &&
        savedDetails['kpiFilters'][tab] &&
        savedDetails['kpiFilters'][tab][subFilter]
          ? savedDetails['kpiFilters'][tab][subFilter]
          : {};
      ...
      savedDetails = {
        ...
        kpiFilters: {
          ...savedDetails['kpiFilters'],
          ...{ [tab]: { [subFilter]: combineSubFilterValues } },
        },
      };
    }
  ...
  setBackupOfFilterSelectionState = (selectedFilterObj) => {
    if (
      ...
      Object.keys(selectedFilterObj)[0] === 'selected_type'
    ) {
    ...
    if (tab === 'backlog') {
      if (
        this.sharedService
          ...
          .hasOwnProperty('kpiFilters') &&
        this.sharedService
          .getAddtionalFilterBackup()
          ['kpiFilters'].hasOwnProperty(tab) &&
        this.sharedService
          .getAddtionalFilterBackup()
          ['kpiFilters'][tab].hasOwnProperty(kpiId)
      ) {
        haveBackup =
          this.sharedService.getAddtionalFilterBackup()['kpiFilters'][tab][
            kpiId
          ];
      }
    } else {
      if (
        ...
        this.sharedService
          .getAddtionalFilterBackup()
          ['kpiFilters'][tab].hasOwnProperty(subFilter)
      ) {
        haveBackup =
          this.sharedService.getAddtionalFilterBackup()['kpiFilters'][tab][
            subFilter
