...
export class HelperService {
  ...
  setBackupOfFilterSelectionState = (selectedFilterObj) => {
    ...
    this.router.navigate([], {
      queryParams: { 'stateFilters': queryParams }, // Pass the object here
      relativeTo: this.route,
      queryParamsHandling: 'merge', // Merge with existing queryParams
    });
    // const stateFilter = JSON.stringify(this.selectedFilters);
    // let url = new URL(window.location.href);
    // url.searchParams.set('stateFilter', stateFilter);
    // this.sharedService.setQueryParams(queryParams);
  }
