...
import {
  UserAccessApprovalDTO,
  UserAccessApprovalResponseDTO,
} from 'src/app/model/userAccessApprovalDTO.model';
...
export class ViewNewUserAuthRequestComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.httpService
      ...
      .subscribe((userData: UserAccessApprovalResponseDTO) => {
    ...
    this.httpService
      ...
      .subscribe((data) => {
        ...
        if (data.success) {
          ...
          this.newUserAccessRequestData = this.newUserAccessRequestData.filter(
            (userData) => userData.username !== req.username,
          );
