...
export class KpiCardV2Component implements OnInit, OnChanges {
  ...
  showWarning(val) {
    if (val) {
      ...
      } else if (this.kpiDataStatusCode === '202') {
  ...
  checkIfDataPresent(data) {
    if ((data === '200' || data === '201'  || data === '202') && (this.kpiData?.kpiId === 'kpi148' || this.kpiData?.kpiId === 'kpi146')) {
      if (this.trendValueList?.length) {
        return true;
      }
    }
    if ((data === '200' || data === '201'  || data === '202') && (this.kpiData?.kpiId === 'kpi139' || this.kpiData?.kpiId === 'kpi127')) {
      if (this.trendValueList?.length && this.trendValueList[0].value?.length) {
        return true;
      }
    }
    if ((data === '200' || data === '201'  || data === '202') && (this.kpiData?.kpiId === 'kpi168' || this.kpiData?.kpiId === 'kpi70' || this.kpiData?.kpiId === 'kpi153'  || this.kpiData?.kpiId === 'kpi35')) {
      if (this.trendValueList?.length && this.trendValueList[0]?.value?.length > 0) {
        return true;
      }
    }

    if ((data === '200' || data === '201'  || data === '202') && (this.kpiData?.kpiId === 'kpi171')) {
    ...
    return (data === '200' || data === '201'  || data === '202') && this.helperService.checkDataAtGranularLevel(this.trendValueList, this.kpiData.kpiDetail.chartType, this.selectedTab);
  }
