...
export class PrimaryFilterComponent implements OnChanges {
      ...
      if (
        ...
        if (
          ...
          let addtnlStateFilters =
            this.helperService.getBackupOfFilterSelectionState(
              'additional_level',
            );
          if (
            addtnlStateFilters &&
  ...
  setProjectAndLevelBackupBasedOnSelectedLevel() {
    if (typeof this.selectedLevel === 'string') {
      ...
      this.service.setSelectedLevel({
        hierarchyLevelName: this.selectedLevel?.toLowerCase(),
      });
    } else {
      this.service.setSelectedTrends(this.selectedLevel['fullNodeDetails']);
      this.service.setSelectedLevel({
        hierarchyLevelName: this.selectedLevel['nodeType']?.toLowerCase(),
      });
    }
  }
  ...
  setDropdownWithMoreActiveOption(selectedLevel) {
    const moreThanOneActiveOption = this.helperService
      .sortByField(
        this.filterData[selectedLevel]?.filter(
          (filter) => filter.parentId === this.selectedLevel.nodeId,
        ),
