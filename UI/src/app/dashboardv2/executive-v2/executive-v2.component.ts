...
import issueHygiene from '../../../assets/data/issue-hygiene-v2.json';
...
export class ExecutiveV2Component implements OnInit, OnDestroy {
  ...
  setFilterValueIfAlreadyHaveBackup(
    ...
    if (this.selectedTab !== 'backlog') {
      ...
      if (
        this.updatedConfigGlobalData.filter((kpi) => kpi?.kpiId == kpiId)[0]
          ?.kpiDetail?.chartType
      ) {
    ...
    if (
      ...
      if (this.service.getSelectedTrends()?.length === 1) {
            ...
            if (Array.isArray(data[0].value) && data[0].value) {
              data[0].value.forEach((element) => {
                if (kpi.kpiId === 'kpi70') {
                  ...
                  Array.isArray(element.dataValue) &&
                  element.dataValue.length
                ) {
        ...
        if (dataValue > 0) {
          ...
          let processorLastRunSuccess = this.showExecutionDate(
