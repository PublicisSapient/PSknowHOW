...
@Service
@Slf4j
public class KanbanHandleAzureIssueHistory {

    public static final String NEW_VALUE = "oldValue";
    public static final String OLD_VALUE = "newValue";
    public static final String DISPLAY_NAME = "displayName";
    private List<JiraHistoryChangeLog> getJiraFieldChangeLogFromAdditionProps(List<Value> updateValueList, String jiraField) {
            ...
            for (Value history : updateValueList) {
                ...
                if (changelogItem != null && changelogItem.getSystemState() != null) {
                    ...
                    jiraHistoryChangeLog
                            .setUpdatedOn(LocalDateTime.parse(AzureProcessorUtil.getFormattedDate(AzureProcessorUtil
    ...
    private List<JiraHistoryChangeLog> getDueDateChangeLog(List<Value> changeLogList, FieldMapping fieldMapping, Map<String, Object> fields) {
        if (StringUtils.isNotEmpty(fieldMapping.getJiraDueDateField())) {
            ...
            if (fieldMapping.getJiraDueDateField().equalsIgnoreCase(CommonConstant.DUE_DATE))
                ...
                IssueField issueField = (IssueField) fields.get(fieldMapping.getJiraDueDateCustomField());
    ...
    public void setJiraIssueCustomHistoryUpdationLog(KanbanIssueCustomHistory jiraIssueCustomHistory, List<Value> updateValueList, FieldMapping fieldMapping,
                                                     Map<String, Object> fieldsMap) {
        List<JiraHistoryChangeLog> statusChangeLog = getStatusChangeLog(updateValueList);
        List<JiraHistoryChangeLog> assigneeChangeLog = getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.ASSIGNEE);
        List<JiraHistoryChangeLog> priorityChangeLog = getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.PRIORITY);
        List<JiraHistoryChangeLog> labelsChangeLog = getJiraFieldChangeLogFromAdditionProps(updateValueList, AzureConstants.LABEL);
