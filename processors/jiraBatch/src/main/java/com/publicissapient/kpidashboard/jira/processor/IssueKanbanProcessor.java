...
public class IssueKanbanProcessor implements ItemProcessor<ReadData, CompositeResult> {
    ...
    private KanbanJiraIssue convertIssueToKanbanJiraIssue(ReadData readData) throws JSONException {
        return kanbanJiraIssueProcessor.convertToKanbanJiraIssue(readData.getIssue(), readData.getProjectConfFieldMapping(), readData.getBoardId());
    }
    ...
    private Set<KanbanAccountHierarchy> createKanbanAccountHierarchies(KanbanJiraIssue kanbanJiraIssue, ReadData readData) {
    ...
    private AssigneeDetails createAssigneeDetails(ReadData readData, KanbanJiraIssue kanbanJiraIssue) {
        return kanbanJiraIssueAssigneeProcessor.createKanbanAssigneeDetails(readData.getProjectConfFieldMapping(), kanbanJiraIssue);
    }
