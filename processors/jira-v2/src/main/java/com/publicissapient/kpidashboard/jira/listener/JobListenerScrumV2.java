...
@Slf4j
@JobScope
public class JobListenerScrumV2 implements JobExecutionListener {
...
//				sendNotification(stepFaliureException);
	...
	private void setExecutionInfoInTraceLog(boolean status) {
		Optional<ProcessorExecutionTraceLog> procExecTraceLogs = processorExecutionTraceLogRepo
				.findByProcessorNameAndBasicProjectConfigId(ProcessorConstants.JIRA_V2, projectId);
		if (procExecTraceLogs.isPresent()) {
			ProcessorExecutionTraceLog processorExecutionTraceLog = procExecTraceLogs.get();
			processorExecutionTraceLog.setExecutionEndedAt(System.currentTimeMillis());
			processorExecutionTraceLog.setExecutionSuccess(status);
			processorExecutionTraceLogRepo.save(processorExecutionTraceLog);
		}
	}
}
