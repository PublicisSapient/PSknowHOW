...
@Slf4j
@Component
public class IssueScrumWriterV2 implements ItemWriter<CompositeResult> {
...
//	private AssigneeDetailsRepository assigneeDetailsRepository;
	...
	public void write(Chunk<? extends CompositeResult> compositeResults) throws Exception {
		Map<String, JiraIssueV2> jiraIssues = new HashMap<>();
...
//		Map<String, AssigneeDetails> assigneesToSave = new HashMap<>();
		Set<SprintDetailsV2> sprintDetailsSet = new HashSet<>();
...
//			}
			if (null != compositeResult.getSprintDetailsV2Set()) {
				sprintDetailsSet.addAll(compositeResult.getSprintDetailsV2Set());
			}
...
//		}
	...
	private void writeJiraItem(Map<String, JiraIssueV2> jiraItems) {
		log.info("Writing issues to Jira_Issue Collection");
		List<JiraIssueV2> jiraIssues = new ArrayList<>(jiraItems.values());
		jiraIssueV2Repository.saveAll(jiraIssues);
	}
...
//	}
	...
	private void writeSprintDetail(Set<SprintDetailsV2> sprintDetailsSet) {
		log.info("Writing issues to SprintDetails Collection");
		SprintV2Repository.saveAll(sprintDetailsSet);
	}
