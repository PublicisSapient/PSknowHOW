...
@Component
@Slf4j
public class JiraIssueV2JqlWriterListener implements ItemWriteListener<CompositeResult> {
	...
	public void afterWrite(Chunk<? extends CompositeResult> compositeResults) {
		...
		Map<String, List<JiraIssueV2>> projectWiseIssues = jiraIssues.stream()
				.collect(Collectors.groupingBy(JiraIssueV2::getBasicProjectConfigId));

		for (Map.Entry<String, List<JiraIssueV2>> entry : projectWiseIssues.entrySet()) {
			String basicProjectConfigId = entry.getKey();
			JiraIssueV2 firstIssue = entry.getValue().stream().max(Comparator
                    .comparing((JiraIssueV2 jiraIssue) -> LocalDateTime.parse(jiraIssue.getChangeDate(), DateTimeFormatter.ofPattern(JiraConstants.JIRA_ISSUE_CHANGE_DATE_FORMAT)))).orElse(null);
			if (firstIssue != null) {
				Optional<ProcessorExecutionTraceLog> procTraceLog = processorExecutionTraceLogRepo
						.findByProcessorNameAndBasicProjectConfigId(ProcessorConstants.JIRA_V2, basicProjectConfigId);
