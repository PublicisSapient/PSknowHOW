...
@Component
@StepScope
public class SprintV2ScrumTasklet implements Tasklet {
	...
	public RepeatStatus execute(StepContribution sc, ChunkContext cc) throws Exception {
		...
		try (ProcessorJiraRestClient client = jiraClient.getClient(projConfFieldMapping, krb5Client)) {
			...
			for (BoardDetails boardDetails : boardDetailsList) {
				List<SprintDetailsV2> sprintDetailsList = fetchSprintV2Report
						.createSprintV2DetailBasedOnBoard(projConfFieldMapping, krb5Client, boardDetails);
				sprintV2Repository.saveAll(sprintDetailsList);
			}
		}
