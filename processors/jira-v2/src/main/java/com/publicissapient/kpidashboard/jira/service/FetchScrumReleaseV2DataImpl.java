...
@Slf4j
@Service
public class FetchScrumReleaseV2DataImpl implements FetchScrumReleaseV2Data {
...
//	private HierarchyLevelService hierarchyLevelService;
	...
	@Override
	public void processReleaseV2Info(ProjectConfFieldMapping projectConfig, KerberosClient krb5Client)
	...
	private void saveProjectRelease(ProjectConfFieldMapping confFieldMapping, AccountHierarchy accountHierarchy, KerberosClient krb5Client) throws IOException, ParseException {
		List<ProjectVersionV2> projectVersionList = jiraCommonService.getMasterVersion(confFieldMapping, krb5Client);
		if (CollectionUtils.isNotEmpty(projectVersionList)) {
			if (null != accountHierarchy) {
				ProjectReleaseV2 projectRelease = projectReleaseV2Repo.findByConfigId(accountHierarchy.getBasicProjectConfigId());
				projectRelease = projectRelease == null ? new ProjectReleaseV2() : projectRelease;
...
//				saveScrumAccountHierarchy(accountHierarchy, confFieldMapping, projectRelease);
				projectReleaseV2Repo.save(projectRelease);
			}
			log.debug("Version processed {}",
					projectVersionList.stream().map(ProjectVersionV2::getName).collect(Collectors.toList()));
		}
	}

/*
	private void saveScrumAccountHierarchy(AccountHierarchy projectData, ProjectConfFieldMapping projectConfig,
			ProjectReleaseV2 projectRelease) {
...
/**
	...
	private List<AccountHierarchy> createScrumHierarchyForRelease(ProjectReleaseV2 projectRelease,
