...
public class SprintClientImpl implements SprintClient {
			...
			sprintDetailsSet.forEach(fetchedSprintDetails -> {
				...
				if (Objects.isNull(dbSprintDetails)) {
					...
					if (fetchedSprintDetails.getState().equalsIgnoreCase(SprintDetails.SPRINT_STATE_ACTIVE) &&
						...
						if (Boolean.TRUE.equals(projectConfig.getProjectToolConfig().getAzureRefreshActiveSprintReport())) {
							...
							toBeSavedSprintDetails.add(fetchedSprintDetails);
							log.debug("Active sprint Id -> {} , toBeSavedCompletedIssues -> {} , toBeSavedNotCompletedIssues -> {}",
									fetchedSprintDetails.getSprintID(), getIssuesIdList(toBeSavedCompletedIssues),
									getIssuesIdList(toBeSavedNotCompletedIssues));
						}
					}
					...
					if (fetchedSprintDetails.getState().equalsIgnoreCase(SprintDetails.SPRINT_STATE_CLOSED) &&
						...
						fetchedSprintDetails.setId(dbSprintDetails.getId());
						toBeSavedSprintDetails.add(fetchedSprintDetails);
						log.debug("fetched sprint Id -> {} , toBeSavedCompletedIssues -> {} , toBeSavedNotCompletedIssues -> {}",
								fetchedSprintDetails.getSprintID(), getIssuesIdList(toBeSavedCompletedIssues),
								getIssuesIdList(toBeSavedNotCompletedIssues));
					}
	...
	private void overrideDBSprintReport(ProjectConfFieldMapping projectConfig, AzureAdapter azureAdapter,
			AzureServer azureServer, SprintDetails fetchedSprintDetails, List<String> completedIssuesStatus,
			ObjectId azureProcessorId, SprintDetails dbSprintDetails) {
		List<SprintIssue> fetchedSprintWiseIssues = fetchAndPrepareSprintIssue(azureAdapter, azureServer, projectConfig,
				fetchedSprintDetails);

		Set<SprintIssue> toBeSavedCompletedIssues = new HashSet<>();
		Set<SprintIssue> toBeSavedNotCompletedIssues = new HashSet<>();
		Set<SprintIssue> toBeSavedTotalIssues = new HashSet<>();
		prepareCompletedAndNotCompletedSprintIssue(completedIssuesStatus, fetchedSprintWiseIssues, toBeSavedCompletedIssues,
				toBeSavedNotCompletedIssues, toBeSavedTotalIssues);
	...
	private void preparePuntedSprintIssues(SprintDetails dbSprintDetails, List<SprintIssue> fetchedSprintWiseIssues,
		...
		if (CollectionUtils.isNotEmpty(dbSprintDetails.getTotalIssues())) {
			...
			Set<SprintIssue> removedIssues = dbSprintDetails.getTotalIssues().stream().filter(Objects::nonNull)
					.filter(sprintIssue -> !fetchedTotalIssues.contains(sprintIssue.getNumber())).collect(Collectors.toSet());
	...
	private void prepareAddedSprintIssues(SprintDetails dbSprintDetails, List<SprintIssue> fetchedSprintWiseIssues,
		...
		if (CollectionUtils.isNotEmpty(dbSprintDetails.getTotalIssues())) {
			...
			// count in total list.
			if (CollectionUtils.isNotEmpty(dbSprintDetails.getPuntedIssues()) && CollectionUtils.isNotEmpty(addedIssues)) {
				...
				Set<SprintIssue> puntedAndAddedWithInSprint = dbPuntedIssues.stream().filter(Objects::nonNull)
						.filter(sprintIssue -> addedIssues.contains(sprintIssue.getNumber())).collect(Collectors.toSet());
				...
				log.debug("Sprint Id -> {} , puntedAndAddedIds -> {}", dbSprintDetails.getSprintID(), puntedAndAddedIds);
			}
	...
	private void iterationStatusUpdateSPIssuesShuffle(ProjectConfFieldMapping projectConfig,
		...
		if (projectToolConfig.isAzureIterationStatusFieldUpdate() && CollectionUtils.isNotEmpty(completedIssuesStatus)) {
	...
	private List<String> getIssuesIdList(Set<SprintIssue> sprintIssueSet) {
		if (CollectionUtils.isNotEmpty(sprintIssueSet)) {
			return sprintIssueSet.stream().filter(Objects::nonNull).map(SprintIssue::getNumber).collect(Collectors.toList());
		}
