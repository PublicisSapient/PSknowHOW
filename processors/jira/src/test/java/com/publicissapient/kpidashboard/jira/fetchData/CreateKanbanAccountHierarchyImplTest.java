...
public class CreateKanbanAccountHierarchyImplTest {
    ...
    @InjectMocks
    private CreateKanbanAccountHierarchyImpl createKanbanAccountHierarchy;
    ...
    public void createAccountHierarchy(){
        ...
        when(kanbanAccountHierarchyRepo.findByLabelNameAndBasicProjectConfigId(any(),any())).thenReturn(accountHierarchies);
        Assert.assertEquals(1,createKanbanAccountHierarchy.createKanbanAccountHierarchy(kanbanJiraIssues,createProjectConfig()).size());
    }
    ...
    private List<KanbanAccountHierarchy> getMockAccountHierarchy(){
        AccountHierarchiesKanbanDataFactory accountHierarchiesDataFactory= AccountHierarchiesKanbanDataFactory.newInstance("/json/default/account_hierarchy_kanban.json");
    ...
    private List<KanbanAccountHierarchy> getMockAccountHierarchyByLabelNameAndBasicProjectConfigId(){
        AccountHierarchiesKanbanDataFactory accountHierarchiesDataFactory= AccountHierarchiesKanbanDataFactory.newInstance("/json/default/account_hierarchy_kanban.json");
        return accountHierarchiesDataFactory.findByLabelNameAndBasicProjectConfigId(CommonConstant.HIERARCHY_LEVEL_ID_PROJECT,"6335368249794a18e8a4479f");
    }
