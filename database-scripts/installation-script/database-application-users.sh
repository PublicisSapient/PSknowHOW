//db.createUser({user: 'knowhowadmin', pwd: 'admin@123', roles:[{role:'root',db:'admin'}]});
use kpidashboard
//db.createUser({user: 'devadmin', pwd: 'devadmin', roles:[{role:'readWrite', db:'kpidashboard'}]})#!/bin/bash
