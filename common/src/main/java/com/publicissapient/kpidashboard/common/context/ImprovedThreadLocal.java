...
public class ImprovedThreadLocal<T> extends ThreadLocal<T> {
	 ...
	 * entry for every thread local value so this will not affect the performance of
	 * accessing and creating thread local values. This synchronized map is not used
	 * to access ThreadLocal values. It is used to preserve strong references to the
	 * objects because Thread objects will have only weak references to ThreadLocal
	 * objects. WeakHashMap allows values to be garbage collected once no one else
	 * is holding onto the Thread
	 */
	...
	/**
	 * This is weak reference to the set of objects belonging to the current thread.
	 * Using this reference we can access the list of strong references in a
	 * non-synchronous way.
	 */
