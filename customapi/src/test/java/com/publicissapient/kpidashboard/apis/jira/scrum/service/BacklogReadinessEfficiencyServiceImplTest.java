...
public class BacklogReadinessEfficiencyServiceImplTest {
	...
	public void setup() {
		...
		kpiRequest = kpiRequestFactory.findKpiRequest("kpi135");
	...
	public void testGetKpiDataProject_closedSprint() throws ApplicationException {
		...
		when(jiraHistoryRepo.findByStoryIDIn(any())).thenReturn(jiraIssueCustomHistories);
		...
		doNothing().when(velocityServiceHelper).getSprintForProject(any(), any(), any(), any());
		...
		try {
			KpiElement kpiElement = backlogReadinessEfficiencyServiceImpl.getKpiData(kpiRequest, kpiRequest.getKpiList().get(0),
