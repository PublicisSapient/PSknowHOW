...
public class TestExecutionServiceImplTest {
    ...
    public void fetchKPIDataFromDb() throws ApplicationException {
        ...
        when(testExecutionRepository.findTestExecutionDetailByFilters(Mockito.anyMap(), Mockito.anyMap())).thenReturn(testExecutionList);
        Map<String, AdditionalFilterCategory> addFilterCategory =  additionalFilterCategoryList.stream().collect(Collectors.toMap(entry -> entry.getFilterCategoryId(), entry -> entry));
        when(filterHelperService.getAdditionalFilterHierarchyLevel()).thenReturn(addFilterCategory);
        Map<String, Object> defectDataListMap = testExecutionServiceImpl.fetchKPIDataFromDb(leafNodeList,
                null, null, kpiRequest);
        Map<String, Object> outputMap = new HashMap<>();
        outputMap.put(TEST_EXECUTION_DETAIL, testExecutionList);
        assertThat("fetch KPI data from DB :", defectDataListMap,
                equalTo(outputMap));
    ...
    public void testGetKpiData() throws ApplicationException {
        ...
        when(testExecutionRepository.findTestExecutionDetailByFilters(Mockito.any(), Mockito.any())).thenReturn(testExecutionList);
        fetchKPIDataFromDb();
        try {
            KpiElement kpiElement = testExecutionServiceImpl.getKpiData(kpiRequest, kpiRequest.getKpiList().get(0),
                    treeAggregatorDetail);
            assertThat("Test Exceution Value :", ((List<DataCount>) ((List<DataCount>) kpiElement.getTrendValueList()).get(0).getValue()).size(),
                    equalTo(5));
