...
public class KPIExcelUtility {
					...
					if (null != jiraIssue) {
						...
						if (StringUtils.isNotEmpty(fieldMapping.getEstimationCriteria())
							...
							Double originalEstimateInHours =
									Double.valueOf(jiraIssue.getOriginalEstimateMinutes()) / 60;
							excelData.setStoryPoint(
									originalEstimateInHours / fieldMapping.getStoryPointToHourMapping() + "/"
	...
	public static void populateDefectRCARelatedExcelData(String sprint, List<JiraIssue> jiraIssues,
		...
		if (CollectionUtils.isNotEmpty(jiraIssues)) {
			jiraIssues.stream().forEach(jiraIssue -> {
				...
				if (null != jiraIssue.getOriginalEstimateMinutes() && StringUtils.isNotEmpty(
						fieldMapping.getEstimationCriteria()) && fieldMapping.getEstimationCriteria()
	...
	public static void populateSonarKpisExcelData(String projectName, List<String> jobList,
		...
		if (CollectionUtils.isNotEmpty(jobList)) {
			for (int i = 0; i < jobList.size(); i++) {
				...
				if (kpiId.equalsIgnoreCase(KPICode.UNIT_TEST_COVERAGE.getKpiId()) || kpiId.equalsIgnoreCase(
						KPICode.UNIT_TEST_COVERAGE_KANBAN.getKpiId())) {
					excelData.setUnitCoverage(kpiSpecificDataList.get(i));
				}
				if (kpiId.equalsIgnoreCase(KPICode.SONAR_TECH_DEBT.getKpiId()) || kpiId.equalsIgnoreCase(
						KPICode.SONAR_TECH_DEBT_KANBAN.getKpiId())) {
					excelData.setTechDebt(kpiSpecificDataList.get(i));
				}
				if (kpiId.equalsIgnoreCase(KPICode.SONAR_VIOLATIONS.getKpiId()) || kpiId.equalsIgnoreCase(
						KPICode.SONAR_VIOLATIONS_KANBAN.getKpiId())) {
	...
	private static void setSonarKpiWeekDayMonthColumn(String versionDate, KPIExcelData excelData, String kpiId) {
		...
		if (kpiId.equalsIgnoreCase(KPICode.UNIT_TEST_COVERAGE.getKpiId()) || kpiId.equalsIgnoreCase(
				KPICode.SONAR_TECH_DEBT.getKpiId()) || kpiId.equalsIgnoreCase(KPICode.SONAR_VIOLATIONS.getKpiId())) {
	...
	public static void populateInSprintAutomationExcelData(String sprint, List<TestCaseDetails> allTestList,
		...
		if (CollectionUtils.isNotEmpty(allTestList)) {
			...
			allTestList.forEach(testIssue -> {
				String present = conditionalList.contains(testIssue.getNumber()) ?
						Constant.EXCEL_YES :
	...
	public static void populateCommittmentReliability(String sprint, Map<String, JiraIssue> totalStoriesMap,
			List<JiraIssue> conditionStories, List<KPIExcelData> kpiExcelData, FieldMapping fieldMapping) {
		if (MapUtils.isNotEmpty(totalStoriesMap)) {
			List<String> conditionalList = conditionStories.stream().map(JiraIssue::getNumber)
					.collect(Collectors.toList());
			totalStoriesMap.forEach((storyId, jiraIssue) -> {
		...
		if (CollectionUtils.isNotEmpty(issuesExcel)) {
			issuesExcel.forEach(e -> {
				...
				if (issueWiseDelay.containsKey(e.getNumber())) {
					...
					excelData.setPotentialDelay(iterationPotentialDelay.getPotentialDelay() + "d");
				...
				if (completedIssue.stream().map(JiraIssue::getNumber).collect(Collectors.toList())
					...
					excelData.setActualCompletionDate(LocalDate.parse(e.getUpdateDate().split("\\.")[0],
	...
	public static void populateTestWithoutStoryExcelData(String projectName, Map<String, TestCaseDetails> totalTestMap,
		...
		if (MapUtils.isNotEmpty(totalTestMap)) {
			...
			totalTestMap.forEach((testId, testCaseDetails) -> {
				String isDefectPresent = testWithoutStoryIdList.contains(testId) ?
						Constant.EMPTY_STRING :
						Constant.EXCEL_YES;
				KPIExcelData excelData = new KPIExcelData();
				excelData.setProjectName(projectName);
	...
	public static void populateOpenVsClosedExcelData(String date, String projectName,
		...
		if (CollectionUtils.isNotEmpty(dateWiseIssueTypeList) || CollectionUtils.isNotEmpty(
	...
	public static void populateIterationKpiWithPCD(List<IterationKpiModalValue> overAllmodalValues,
		...
		if (null != jiraIssue.getOriginalEstimateMinutes() && StringUtils.isNotEmpty(
				fieldMapping.getEstimationCriteria()) && fieldMapping.getEstimationCriteria()
				.equalsIgnoreCase(CommonConstant.ACTUAL_ESTIMATION)) {
			iterationKpiModalValue.setIssueSize((jiraIssue.getOriginalEstimateMinutes() / 60 + " hrs"));
		}
		...
		if (issueWiseDelay.containsKey(jiraIssue.getNumber())) {
			...
			iterationKpiModalValue.setPotentialDelay(iterationPotentialDelay.getPotentialDelay() + "d");
	...
	public static void populateWorkRemainingWithPCD(List<IterationKpiModalValue> overAllmodalValues,
		...
		if (issueWiseDelay.containsKey(jiraIssue.getNumber()) && StringUtils.isNotEmpty(jiraIssue.getDueDate())) {
			...
			IterationPotentialDelay iterationPotentialDelay = issueWiseDelay.get(jiraIssue.getNumber());
			iterationKpiModalValue.setPotentialOverallDelay(iterationPotentialDelay.getPotentialDelay() + "d");
	...
	public static void populateIterationDataForQualityStatus(List<IterationKpiModalValue> overAllmodalValues,
		...
		if (CollectionUtils.isNotEmpty(linkedJiraIssueStoryList)) {
			...
			linkedJiraIssueStoryList.forEach(linkedStory -> {
				...
				if (estimationFlag) {
					if (null != linkedStory.getStoryPoints() && StringUtils.isNotEmpty(
							fieldMapping.getEstimationCriteria()) && fieldMapping.getEstimationCriteria()
							.equalsIgnoreCase(CommonConstant.STORY_POINT)) {
						storyPoint.updateAndGet(v -> v + linkedStory.getStoryPoints());
					}
					if (null != linkedStory.getOriginalEstimateMinutes() && StringUtils.isNotEmpty(
							fieldMapping.getEstimationCriteria()) && fieldMapping.getEstimationCriteria()
							.equalsIgnoreCase(CommonConstant.ACTUAL_ESTIMATION)) {
	...
	public static void populateRefinementRejectionExcelData(List<KPIExcelData> excelDataList,
		...
		if (CollectionUtils.isNotEmpty(issuesExcel)) {
			issuesExcel.forEach(e -> {
				...
				Map<String, String> epicLink = new HashMap<>();
				epicLink.put(e.getNumber(), checkEmptyURL(e));
				excelData.setChangeDate(LocalDate.parse(e.getChangeDate().split("\\.")[0],
						DateTimeFormatter.ofPattern(DateUtil.TIME_FORMAT)).toString());
				excelData.setIssueID(epicLink);
