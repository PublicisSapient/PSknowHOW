...
public class ReleasePlanServiceImpl extends JiraReleaseKPIService {
		...
		if (null != leafNode) {
			...
			if (CollectionUtils.isNotEmpty(releaseList)) {
				...
				dateWiseLogs(allIssuesHistory, releaseList.stream().findFirst().orElse(null), releaseIssues,
						addedIssuesMap, removeIssueMap, fullReleaseMap);
	...
	private void populateFilterWiseDataMap(Map<String, List<JiraIssue>> filterWiseGroupedMap, DataCountGroup issueCount,
		...
		processedPlannedIssues.addAll(overallIssues.stream().filter(f -> StringUtils.isNotBlank(f.getDueDate()))
				.distinct().toList());
		...
		if (plannedDueDate != null) {
			if (maxPlannedDueDate == null || plannedDueDate.isAfter(maxPlannedDueDate)) {
