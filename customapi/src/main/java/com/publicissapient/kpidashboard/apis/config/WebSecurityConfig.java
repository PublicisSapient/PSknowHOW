...
public class WebSecurityConfig implements WebMvcConfigurer {
	...
	protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authz -> authz.requestMatchers("/appinfo").permitAll()
				...
				.requestMatchers("/cache/clearAllCache").permitAll().requestMatchers(HttpMethod.GET, "/cache/**")
				.permitAll().requestMatchers(HttpMethod.OPTIONS, "/**").permitAll()
				.requestMatchers(HttpMethod.GET, "/analytics/switch").permitAll()
				.requestMatchers("/stringShortener/shorten").permitAll()
				.requestMatchers("/stringShortener/longString").permitAll().anyRequest().authenticated())
