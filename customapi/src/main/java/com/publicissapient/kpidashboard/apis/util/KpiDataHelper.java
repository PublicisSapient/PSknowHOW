...
public final class KpiDataHelper {
     ...
     */
    public static String createAdditionalFilterMap(KpiRequest kpiRequest, Map<String, List<String>> mapOfFilters, String methodology, String individualDevOrQa, FilterHelperService flterHelperService) {
        ...
        Map<String, AdditionalFilterCategory> addFilterCategory = addFilterCat.entrySet().stream().collect(Collectors.toMap(entry -> entry.getKey().toUpperCase(), Map.Entry::getValue));
     ...
     */
    public static Map<Pair<String, String>, Map<String, List<String>>> createSubCategoryWiseMap(String subGroupCategory, List<SprintWiseStory> sprintWiseStoryList, String filterToShowOnTrend) {

        Map<Pair<String, String>, Map<String, List<String>>> sprintWiseStoryMap = new HashMap<>();

        Map<Pair<String, String>, List<SprintWiseStory>> sprintAndFilterDataMap = sprintWiseStoryList.stream().collect(Collectors.groupingBy(sws -> Pair.of(sws.getProjectID(), sws.getSprint()), Collectors.toList()));
        ...
        sprintAndFilterDataMap.entrySet().forEach(data -> {
            ...
            if (Constant.SPRINT.equals(subGroupCategory)) {
                subCategoryDataMap = data.getValue().stream().collect(Collectors.toMap(SprintWiseStory::getSprint, SprintWiseStory::getStoryList));
            }
     ...
     * @return
     */
    public static Map<String, List<KanbanJiraIssue>> createProjectWiseMapKanban(List<KanbanJiraIssue> ticketList, String subGroupCategory, FilterHelperService flterHelperService) {
        ...
        if (Constant.DATE.equals(subGroupCategory) || addFilterCategoryList.contains(subGroupCategory)) {
            projectAndDateWiseTicketMap = ticketList.stream().collect(Collectors.groupingBy(KanbanJiraIssue::getBasicProjectConfigId));
        }
     ...
     * @return
     */
    public static Map<String, List<KanbanIssueCustomHistory>> createProjectWiseMapKanbanHistory(List<KanbanIssueCustomHistory> ticketList, String subGroupCategory, FilterHelperService flterHelperService) {
        Map<String, List<KanbanIssueCustomHistory>> projectAndDateWiseTicketMap = new HashMap<>();
        Map<String, AdditionalFilterCategory> addFilterCat = flterHelperService.getAdditionalFilterHierarchyLevel();
        List<String> addFilterCategoryList = new ArrayList<>(addFilterCat.keySet());
        if (Constant.DATE.equals(subGroupCategory) || addFilterCategoryList.contains(subGroupCategory)) {
            projectAndDateWiseTicketMap = ticketList.stream().collect(Collectors.groupingBy(KanbanIssueCustomHistory::getBasicProjectConfigId));
        }
     ...
     * @param flterHelperService
     */
    public static Map<String, Map<String, List<KanbanCapacity>>> createDateWiseCapacityMap(List<KanbanCapacity> ticketList, String subGroupCategory, FilterHelperService flterHelperService) {
        Map<String, AdditionalFilterCategory> addFilterCat = flterHelperService.getAdditionalFilterHierarchyLevel();
        List<String> addFilterCategoryList = new ArrayList<>(addFilterCat.keySet());
        Map<String, Map<String, List<KanbanCapacity>>> projectAndDateWiseCapacityMap = new HashMap<>();
        if (Constant.DATE.equals(subGroupCategory) || addFilterCategoryList.contains(subGroupCategory)) {
            Map<String, List<KanbanCapacity>> projectWiseCapacityMap = ticketList.stream().collect(Collectors.groupingBy(ticket -> ticket.getBasicProjectConfigId().toString(), Collectors.toList()));
            projectWiseCapacityMap.forEach((project, capacityList) -> {
                ...
                capacityList.forEach(kanbanCapacity -> {
                    for (LocalDate date = kanbanCapacity.getStartDate(); (date.isBefore(kanbanCapacity.getEndDate()) || date.isEqual(kanbanCapacity.getEndDate())) && !(date.getDayOfWeek().equals(DayOfWeek.SUNDAY) || date.getDayOfWeek().equals(DayOfWeek.SATURDAY)); date = date.plusDays(1)) {
     ...
     * @return
     */
    public static List<String> getIssuesIdListBasedOnTypeFromSprintDetails(SprintDetails sprintDetails, String issueType) {
        if (issueType.equalsIgnoreCase(CommonConstant.COMPLETED_ISSUES)) {
            return CollectionUtils.emptyIfNull(sprintDetails.getCompletedIssues()).stream().filter(Objects::nonNull).map(SprintIssue::getNumber).distinct().collect(Collectors.toList());
        } else if (issueType.equalsIgnoreCase(CommonConstant.NOT_COMPLETED_ISSUES)) {
            return CollectionUtils.emptyIfNull(sprintDetails.getNotCompletedIssues()).stream().filter(Objects::nonNull).map(SprintIssue::getNumber).distinct().collect(Collectors.toList());
        } else if (issueType.equalsIgnoreCase(CommonConstant.PUNTED_ISSUES)) {
            return CollectionUtils.emptyIfNull(sprintDetails.getPuntedIssues()).stream().filter(Objects::nonNull).map(SprintIssue::getNumber).distinct().collect(Collectors.toList());
        } else if (issueType.equalsIgnoreCase(CommonConstant.COMPLETED_ISSUES_ANOTHER_SPRINT)) {
            return CollectionUtils.emptyIfNull(sprintDetails.getCompletedIssuesAnotherSprint()).stream().filter(Objects::nonNull).map(SprintIssue::getNumber).distinct().collect(Collectors.toList());
        } else if (issueType.equalsIgnoreCase(CommonConstant.TOTAL_ISSUES)) {
            return CollectionUtils.emptyIfNull(sprintDetails.getTotalIssues()).stream().filter(Objects::nonNull).map(SprintIssue::getNumber).distinct().collect(Collectors.toList());
        } else {
            return new ArrayList<>();
        }
    }

    public static void prepareFieldMappingDefectTypeTransformation(Map<String, Object> mapOfProjectFilters, FieldMapping fieldMapping, List<String> kpiWiseDefectsFieldMapping, String key) {
        if (Optional.ofNullable(fieldMapping.getJiradefecttype()).isPresent() && CollectionUtils.containsAny(kpiWiseDefectsFieldMapping, fieldMapping.getJiradefecttype())) {
     ...
     * @return
     */
    public static Set<JiraIssue> getFilteredJiraIssuesListBasedOnTypeFromSprintDetails(SprintDetails sprintDetails, Set<SprintIssue> sprintIssues, List<JiraIssue> allJiraIssue) {
        ...
        if (CollectionUtils.isNotEmpty(sprintIssues)) {
            ...
            CollectionUtils.emptyIfNull(sprintDetails.getTotalIssues()).stream().forEach(sprintIssue -> issueKeyWiseSprintIssue.put(sprintIssue.getNumber(), sprintIssue));
            CollectionUtils.emptyIfNull(sprintDetails.getPuntedIssues()).stream().forEach(sprintIssue -> issueKeyWiseSprintIssue.put(sprintIssue.getNumber(), sprintIssue));
        }
    ...
    public static List<IterationPotentialDelay> sprintWiseDelayCalculation(List<JiraIssue> inProgressIssuesJiraIssueList, List<JiraIssue> openIssuesJiraIssueList, SprintDetails sprintDetails) {
        List<IterationPotentialDelay> iterationPotentialDelayList = new ArrayList<>();
        LocalDate pivotPCD = null;
        Map<LocalDate, List<JiraIssue>> dueDateWiseInProgressJiraIssue = createDueDateWiseMap(inProgressIssuesJiraIssueList);
    ...
    private static LocalDate getNextPotentialClosedDate(SprintDetails sprintDetails, List<IterationPotentialDelay> iterationPotentialDelayList, LocalDate pivotPCD, Map.Entry<LocalDate, List<JiraIssue>> entry) {
    ...
    private static LocalDate getPotentialClosedDate(SprintDetails sprintDetails, LocalDate pivotPCD, int estimatedTime) {
        return (estimatedTime==0 && sprintDetails.getState().equalsIgnoreCase(CLOSED)) ? DateUtil.stringToLocalDate(sprintDetails.getEndDate(), DateUtil.TIME_FORMAT_WITH_SEC):createPotentialClosedDate(sprintDetails, estimatedTime, pivotPCD);
    }

    private static IterationPotentialDelay createIterationPotentialDelay(LocalDate potentialClosedDate, int potentialDelay, int remainingEstimateTime, JiraIssue issue, boolean sprintClosed, LocalDate dueDate) {
     ...
     * @return
     */
    private static LocalDate checkPivotPCD(SprintDetails sprintDetails, LocalDate potentialClosedDate, int remainingEstimateTime, LocalDate pivotPCDLocal) {
        if ((pivotPCDLocal==null || pivotPCDLocal.isBefore(potentialClosedDate)) && (!sprintDetails.getState().equalsIgnoreCase(CLOSED) || (sprintDetails.getState().equalsIgnoreCase(CLOSED) && remainingEstimateTime!=0))) {
     ...
     */
    private static LocalDate createPotentialClosedDate(SprintDetails sprintDetails, int remainingEstimateTime, LocalDate pivotPCD) {
        LocalDate pcd = null;
        if (pivotPCD==null) {
            // for the first calculation
            LocalDate startDate = sprintDetails.getState().equalsIgnoreCase(CLOSED) ? DateUtil.stringToLocalDate(sprintDetails.getEndDate(), DateUtil.TIME_FORMAT_WITH_SEC):LocalDate.now();
     ...
     * @return
     */
    public static void arrangeJiraIssueList(FieldMapping fieldMapping, List<JiraIssue> allIssues, List<JiraIssue> inProgressIssues, List<JiraIssue> openIssues) {
        ...
        if (null!=fieldMapping.getJiraStatusForInProgress() && org.apache.commons.collections.CollectionUtils.isNotEmpty(fieldMapping.getJiraStatusForInProgress())) {
            inProgressIssues.addAll(jiraIssuesWithDueDate.stream().filter(jiraIssue -> fieldMapping.getJiraStatusForInProgress().contains(jiraIssue.getStatus())).collect(Collectors.toList()));
            openIssues.addAll(jiraIssuesWithDueDate.stream().filter(jiraIssue -> !fieldMapping.getJiraStatusForInProgress().contains(jiraIssue.getStatus())).collect(Collectors.toList()));
        } else {
     ...
     * @return
     */
    public static Double getOriginalEstimate(List<Double> overAllOriginalEstimate, Double originalEstimate, JiraIssue jiraIssue) {
